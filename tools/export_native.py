...
def main() -> None:
    ...
    golden_tokens = [3, 20, 50, 4, 30, 5, 2]
    with torch.no_grad():
        golden_logits, _ = quantized_model(torch.tensor([golden_tokens]))
    golden_path = output.with_suffix(output.suffix + ".golden.bin")
    golden = struct.pack("<4sHHI", b"CHLG", 1, len(golden_tokens), config.vocab_size)
    golden += struct.pack(f"<{len(golden_tokens)}I", *golden_tokens)
    golden += golden_logits[0, -1].numpy().astype("<f4", copy=False).tobytes()
    golden_path.write_bytes(golden)
    manifest["golden_sha256"] = hashlib.sha256(golden).hexdigest()
    manifest["golden_tokens"] = golden_tokens
