"""Train Circuit Heroes LM with validation checkpointing and early stopping.

Adapted by the Circuit Heroes LM project from slvDev/esp32-ai under MIT.
"""
...
class Batcher:
    def __init__(self, split, batch_size, seq_len, device, suffix="", data_dir=DATA):
        self.data = np.memmap(os.path.join(data_dir, f"{split}{suffix}.bin"), dtype=np.uint16, mode="r")
        mask_path = os.path.join(data_dir, f"{split}{suffix}.answer-mask.bin")
        self.answer_mask = np.memmap(mask_path, dtype=np.uint8, mode="r") if os.path.exists(mask_path) else None
    ...
    def __call__(self):
        ...
        if self.answer_mask is not None:
            masks = np.stack([self.answer_mask[i + 1 : i + 1 + self.sl] for i in ix])
            y[masks == 0] = -1
...
def main():
    ...
    ap.add_argument("--early-stop-evals", type=int, default=0)
    ...
    train_b = Batcher("train", args.batch_size, args.seq_len, device, suffix, args.data_dir)
    val_b = Batcher("validation" if os.path.exists(os.path.join(args.data_dir, "validation.bin")) else "val",
                    args.batch_size, args.seq_len, device, suffix, args.data_dir)
    ...
    for step in range(args.steps):
        ...
        if step % args.eval_every == 0 or step == args.steps - 1:
            ...
            if args.early_stop_evals and bad_evals >= args.early_stop_evals:
                print(f"{name} early stop: no validation improvement for "
                      f"{bad_evals} evaluations", flush=True)
                break
    ...
    result = {
        ...
        "tokens_seen": completed_steps * args.batch_size * args.seq_len,
